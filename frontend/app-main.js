...
function renderModels(f) {
  ...
  g.innerHTML = list.map((m,i) => `<div class="mc ${getCardClass(m)}" onclick="useModel('${m.name}')" style="animation-delay:${i*0.04}s">${m.isNew?'<div class="mc-new">NEU</div>':''}<div class="mc-provider">${m.provider}</div><div class="mc-name">${m.name}</div><div class="mc-mult">x${m.mult}</div><div class="mc-price">€${(m.mult).toFixed(2)} / 1K</div><span class="mc-tag ${m.tagClass}">${m.tag}</span></div>`).join('');
}
...
(async function initApp() {
  if (!_getToken()) { window.location.href = '/login.html'; return; }
