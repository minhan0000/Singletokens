...
const DB_PATH = process.env.NODE_ENV === 'production' ? '/tmp/singletokens.db' : require('path').join(__dirname, 'singletokens.db');
...
module.exports = {
  ...
  gpts: {
    getAll: (userId) => all('SELECT * FROM gpts WHERE user_id = ? ORDER BY created_at DESC', [userId]),
    get: (id, userId) => get('SELECT * FROM gpts WHERE id = ? AND user_id = ?', [id, userId]),
    create: (id, userId, name, description, model, prompt, temp, cap, icon) => run('INSERT INTO gpts (id,user_id,name,description,model,prompt,temp,cap,icon) VALUES (?,?,?,?,?,?,?,?,?)', [id, userId, name, description, model, prompt, temp, cap, icon]),
    update: (name, description, model, prompt, temp, cap, icon, id, userId) => run('UPDATE gpts SET name=?,description=?,model=?,prompt=?,temp=?,cap=?,icon=?,updated_at=datetime("now") WHERE id=? AND user_id=?', [name, description, model, prompt, temp, cap, icon, id, userId]),
    delete: (id, userId) => run('DELETE FROM gpts WHERE id=? AND user_id=?', [id, userId]),
    deleteAll: (userId) => run('DELETE FROM gpts WHERE user_id=?', [userId]),
  },
  transactions: {
